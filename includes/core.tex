\section{Teoretické východiská}
        \subsection{Nomenklátory}
            ...
            \subsubsection{Homofónna substitučná šifra}
                Homofónna substitučná šifra je typ substitučnej šifry, v ktorej môže mať každý znak otvoreného textu viacero rôznych šifrových reprezentácií. Počet možných náhrad je často úmerný frekvencii daného znaku v jazyku, čím sa vyrovnávajú frekvencie symbolov v šifrovanom texte a znižuje sa účinnosť frekvenčnej analýzy. Táto technika premieňa otvorený text na zdanlivo náhodnú sekvenciu znakov \cite{vatsa2012novel}. V nasledujúcich kapitolách sa budeme venovať zastúpeniam týchto šifier v nomenklátoroch \cite{antal2022wrong}.
...
Príprava kvalitného datasetu predstavuje dôležitý krok v úspešnom vývoji systému umelej inteligencie pre detekciu historických rukou písaných šifrovacích kľúčov. Táto kapitola detailne opisuje proces zhromažďovania a spracovania dát potrebných na trénovanie modelov hlbokého učenia. Osobitná pozornosť je venovaná špecifikám nomenklátorových šifier a ich charakteristikám. Tie tvoria základ pre definovanie tried pre detekciu. Proces anotácie je navrhnutý tak, aby zachytil štruktúru týchto jednotlivých tried. 
    ...
    Finálna dátová sada obsahuje 89 digitalizovaných strán historických nomenklátorových šifrovacích kľúčov, pričom každá strana predstavuje samostatnú jednotku pre detekciu. Každý obrázok zachytáva inú veľkosť a množstvo elementov, čo prispieva k rôznorodosti datasetu.
    Dátová sada je charakterizovaná značnou heterogenitou vo vizuálnom usporiadaní dokumentov. Dokumenty obsahujú hlavné typy substitučných podsystémov, ktorými sú písmená, n-gramy, klamáče a kódy. Dataset odráža historickú realitu používania týchto šifier v praxi.
    ...
    Nasledujúce podkapitoly sa zameriavajú na podrobný opis výberu tried objektov, proces anotácie a spracovanie datasetu.
        ...
        Trieda "section" predstavuje ohraničujúce sekcie, ktoré rozdeľujú jednotlivé časti kľúča.
            ...
            \begin{figure}[H]
                ...
                \caption{Ukážka obrázku z datasetu s detailnejším pohľadom na anotované objekty}
    ...
    Dátová sada bola rozdelená na trénovaciu a validačnú množinu v pomere 80:20. Trénovacia množina obsahuje 69 dokumentov a slúži na optimalizáciu parametrov neurónových sietí. Validačná množina s 18 dokumentmi je používaná na ladenie hyperparametrov a monitorovanie procesu trénovania s cieľom predchádzať pretrénovaniu. Testovacia množina s 37 dokumentmi zostáva počas celého procesu vývoja nedotknutá a slúži na finálne vyhodnotenie výkonnosti modelov.
    
    Rozdelenie bolo realizované skriptom poskytnutým samotným YOLO, aby sa zabezpečilo kompatibilné a rovnomerné zastúpenie dokumentov.
...
V tejto kapitole je popísaný proces implementácie a experimentálneho overenia detekčných modelov na úlohe detekcie komponentov historických rukou písaných nomenklátorových šifrovacích kľúčov. Cieľom experimentov bolo porovnať výkonnosť rôznych variantov modelu YOLO na viacerých verziách datasetu (pôvodný, binarizovaný, augmentovaný) a vyhodnotiť vplyv jednotlivých spracovaní na presnosť detekcie. Kapitola sa venuje výberu nástrojov,  konfigurácii modelov, nastaveniu tréningových parametrov, priebehu tréningu a použitiu hodnotiacich metrík.
    ...
    Výpočtové experimenty prebiehali na zariadení s grafickou kartou NVIDIA TESLA T4 (16GB VRAM). Tento hardvér umožnil efektívny tréning aj najväčších variantov modelu YOLOv11 v krátkom čase.
    
    Pre spracovanie a predspracovanie datasetu boli využité knižnice OpenCV, Albumentations (pre augmentácie) a NumPy. Binarizácia bola realizovaná pomocou YOLO priamo pri trénovaní.
    ...
    Model YOLOv11 je v čase písania tejto práce najnovšou generáciou objektových detektorov, ktorá prináša výrazné zlepšenia v rýchlosti a presnosti oproti predchádzajúcim verziám. Pre potreby tejto práce bola použitá konfigurácia pre úlohu detekcie objektov s viactriedovou segmentáciou podľa definovaných tried.
    ...
    \subsection{Výber modelu}
            ...
            Stredné a veľké modely (yolo11m.pt, yolo11l.pt, yolo11x.pt), napriek ich veľkosti, nedosiahli väčšiu presnosť. Ich tréning bol náchylnejší na pretrénovanie a vzhľadom na ich náročnosť sa musela pamäť uvoľniť znížením hodnôt premenných, ktoré zefektívneli trénovanie na menších modeloch.
    ...
    \subsection{Experimenty s augmentáciou datasetu}
            ...
            Pri experimentoch v tejto kapitole bol model YOLO11s trénovaný na maximálne 200 epoch, pričom bol zapnutý mechanizmus včasného zastavenia (early stopping) s nastavenou hodnotou patience na 20.
                ...
                Tréning zahŕňal maximálne 200 epoch a bol kontrolovaný mechanizmom predčasného zastavenia (early stopping) s hodnotou patience 20. Proces trenovania sa zastavil po dosiahnutí 111 epoch. Dosiahnuté výsledky zahrňovali hodnotu mAP@0.5 na úrovni 0.573 a recall 0.566.
            ...
            \subsection{Experimenty s počtom epoch 300 a patience 50}
                ...
                Na základe uvedených experimentov možno skonštatovať, že kombinácia miernej augmentácie, vyššieho počtu epoch a menej prísneho mechanizmu early stopping predstavuje najefektívnejšiu stratégiu. Môže však rozhodujúcim parametrom byť, že zbehnutie všetkych 300 epoch je lepšie než predčasné ukončenie tréningu, čo umožňuje modelu plne využít potenciál. 
...
Táto kapitola prezentuje výsledky trénovania modelov YOLO11s na rôznych verziách datasetu historických rukou písaných šifrovacích kľúčov. O predčasné ukončenie učenia sa stará parameter patience, ktorý predstavuje počet epoch bez zlepšenia. Cieľom bolo ukázať vplyv predspracovania na presnosť detekcie a porovnať výkonnosť jednotlivých modelov v týchto podmienkach bez ohľadu na dĺžku trénovania. Výsledky sú prezentované pomocou metrík ako mAP, precision, recall a F1-score aj kvalitatívne, s dôrazom na praktickú použiteľnosť detekčných výstupov.
    ...
    \begin{figure}[H]
        ...
        \caption{confusion matica k najlepšiemu meraniu (augemntácia s počtom epoch 300 a patience 50) - model YOLO11s}
        \label{fig:matrix}
    \end{figure}
    ...
    \begin{figure}[H]
        ...
        \caption{Niektoré prvky boli ťažko oddeliteľné, čo viedlo k nepresnému označeniu a ťažšiemu rozpoznaniu.}
        \label{fig:zasahy}
    \end{figure}
